...
Class SliceWP_Object_DB_Visits extends SliceWP_Object_DB {
	...
	public function get_visits( $args = array(), $count = false ) {
		...
		// Include commission_id in where clause.
		if ( ! empty( $args['commission_id'] ) ) {

			$commission_id =  absint( $args['commission_id']);
			$where  .= " AND commission_id = '{$commission_id}' ";

		}

		// Include "ip_address" in where clause.
		...
		// Include "landing_url" in where clause.
		if ( ! empty( $args['landing_url'] ) ) {

			$landing_url = sanitize_text_field( $args['landing_url'] );
			$where 	    .= " AND landing_url = '{$landing_url}' ";
		...
		if ( ! empty( $args['search'] ) ) {
			...
			$where  .= " AND (affiliate_id IN({$affiliate_ids}) OR ip_address LIKE '%%{$search}%%' OR landing_url LIKE '%%{$search}%%' OR referrer_url LIKE '%%{$search}%%')";
		...
		// Callback.
		$callback = ( $fields == '*' ? 'slicewp_get_visit' : '' );

...
function slicewp_list_table_commissions_add_reference_edit_link_edd( $output, $item ) {
    ...
    if ( ! empty( $payment->ID ) )
		$output = '<a href="' . add_query_arg( array( 'post_type' => 'download', 'page' => 'edd-payment-history', 'view' => 'view-order-details', 'id' => $item['reference'] ), admin_url( 'edit.php' ) ) . '">' . $item['reference'] . '</a>';
